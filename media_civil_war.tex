...
\subsection{Research Strategy, Data, and
Method}\label{research-strategy-data-and-method}

This section outlines the overall analytical strategy designed to weigh
the general pacification theory (levels of mass media density
monotonically decrease the likehlihood of civil war onset) with the
war-before-peace theory (early increases in mass media density increase
the likelihood of civil war before levels of mass media density decrease
it in the long-run).
...
To balance these trade-offs, the first part of the analysis employs a
combination of classic graphical analysis and semi-parametric
regressions. Graphical analysis is used to explore several features of
the distributions of mass media density and civil war relevant to
...
tentatively confirmed and the subsequent analyses will build on this
first, somewhat inductive set of analyses.

is interpretively inconvenient and inferentially problematic as they do
not , as essentially allowing for knots in the relationship between an
independent and dependent variable and providing the analyst with a
non-linear function which is smoothed according to penalized regression
splines.

First, graphical analysis is used to reveal whether there is a
non-trivial subset of civil war onsets characterized by low but positive
mass media density, compared to civil war onsets characterized by zero
mass media density, and to. The general pacification theory implies that
m

As observable implication \#2,

Second, as observable implication \#3 relates to a different level of
analysis than the baseline analyses, I collect additional,
international-level data to control for potential confounders. Although
there is not potential confounders

as the previous section details, three observable implications which
either contradict or are not predicted by the pacification theory have
been delineated. We should be convinced of the new, war-before-peace
theory if it explains significant cross-national and/or temporal
variation in the onset of civil wars unaccounted for by the general
pacification theory, while still explaining that portion of the variance
already explained by the pacification theory. Observable implications 1
and 2 are testable with the original data used by Warren, requiring only
the generation of differenced variables which capture year-to-year
changes rather than levels as in the original analyses.

To test these implications, I pursue the following sequence of analyses.

For the modified theory to be preferable to the original theory, this
subset of country-years should be non-trivial in size

Second, I increase data-set observations by extending the original
sample.(Brady and Collier 2010, 184; G King, Keohane, and Verba 1994,
30).
